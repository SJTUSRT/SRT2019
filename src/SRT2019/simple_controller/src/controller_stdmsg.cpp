...
void Callback_control(const geometry_msgs::Twist &msg){
    if(msg.linear.x>0.0){
        ...
        //vco[2].ID = 0x03;
		vco[2].ID = 0x01;	
        vco[2].SendType = 0;// 正常发送
        vco[2].RemoteFlag = 0;// 数据帧
        vco[2].ExternFlag = 0;// 标准帧
        vco[2].DataLen = 8;
        if(msg.angular.z > 0){
            vco[2].Data[0] = 0x01;
            vco[2].Data[1] = 0x00;
        ...
        if(!VCI_Transmit(gDevType, gDevIdx, 0, vco,2)){
            ROS_ERROR_STREAM("Transmit fail");
            return;
        }
        else{
            ROS_INFO_STREAM("Transmit succeed");
        }
    }
    else{
        VCI_CAN_OBJ vco[2];
...
int main(int argc, char **argv){
    ...
    ros::NodeHandle nh;
    ros::Subscriber sub = nh.subscribe("/cmd_vel_mux/teleop", 1, Callback_control);
